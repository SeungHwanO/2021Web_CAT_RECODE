...
function getMovies(url) {
    ...
    fetch(url).then(res => res.json()).then(data => {
        const list = []; // movies라는 배열선언
        for (var i = 0; i < 5; i++) {
            list[i] = data.results[i]; // movies라는 배열에 결과 값을 선언
        }
        showlist(list) // 다음 기능에 movies배열을 넘김
    })
}


function showlist(list) {
    document.getElementById('item_list_layout').innerHTML = ''; // getElementById('[id]')로 html에서 main이라는 id 뒤에 개행

    list.forEach(list => {
        const { id } = list; // api post로 날려 message로 받은 'id'라는 값을 무비로부터 받음
